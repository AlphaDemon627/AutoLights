 ...
 */
@Entity
@Table(name = "tuesday", catalog = "autolightsdb2", schema = "")
@NamedQueries({
    @NamedQuery(name = "Tuesday.findAll", query = "SELECT t FROM Tuesday t"),
    @NamedQuery(name = "Tuesday.findByEntryId", query = "SELECT t FROM Tuesday t WHERE t.entryId = :entryId"),
    @NamedQuery(name = "Tuesday.findByMovementId", query = "SELECT t FROM Tuesday t WHERE t.movementId = :movementId"),
    @NamedQuery(name = "Tuesday.findByTimeId", query = "SELECT t FROM Tuesday t WHERE t.timeId = :timeId"),
    @NamedQuery(name = "Tuesday.findByDay", query = "SELECT t FROM Tuesday t WHERE t.day = :day"),
    @NamedQuery(name = "Tuesday.findByDate", query = "SELECT t FROM Tuesday t WHERE t.date = :date"),
    @NamedQuery(name = "Tuesday.findByLgvCount", query = "SELECT t FROM Tuesday t WHERE t.lgvCount = :lgvCount"),
    @NamedQuery(name = "Tuesday.findByHgvCount", query = "SELECT t FROM Tuesday t WHERE t.hgvCount = :hgvCount"),
    @NamedQuery(name = "Tuesday.findByTotalVehCount", query = "SELECT t FROM Tuesday t WHERE t.totalVehCount = :totalVehCount")})
public class Tuesday implements Serializable {
    ...
    public Tuesday(Integer entryId) {
    ...
    public String toString() {
        return "AutoLightsUI.Tuesday[ entryId=" + entryId + " ]";
    }
